...
<?php foreach ($items as $key => $item): ?>
	<option value="<?php echo $item->{$options['render']['value_field']} ?>" <?php echo ($model->has($relation, $item) OR in_array($item->id, $selected_items)) ? 'selected' : '' ?> style="padding: <?php echo $lvl ?>rem"><?php echo $item->get_prop($options['render']['title_field']) ?></option>
	<?php
		if (isset($options['nested']))
		{
